...
**NOTE: This is a replica yet an improved version to [socketio-auth](https://www.npmjs.com/package/socketio-auth) where an acknowledgement is expected for authenticated emit in order to proceed the server side code to postAuthenticate handler. Since [socketio-auth](https://www.npmjs.com/package/socketio-auth) is read-only and not maintained anymore by the author, this package is created explicitly for a better usage.**

Client:
```javascript
var socket = io.connect('http://localhost');
socket.on('connect', () => {
  socket.emit('authentication', {username: "Sockenticate User", password: "secret"});
  socket.on('authenticated', (data, respond) => {
    if (respond) {
      respond ({ status: true })  // This is the acknowledgement to authenticated emit so that server can proceed the authenticated user to postAuthenticate handler
    }
...
To setup authentication for the socket.io connections, just pass the server socket to sockenticate with a configuration object:
...
**sockenticate** implements two-step authentication: upon connection, the server marks the clients as unauthenticated and listens to an `authentication` event. If a client provides wrong credentials or doesn't authenticate after a timeout period it gets disconnected. While the server waits for a connected client to authenticate, it won't emit any broadcast/namespace events to it. By using this approach the sensitive authentication data, such as user credentials or tokens, travel in the body of a secure request, rather than a querystring that can be logged or cached.
